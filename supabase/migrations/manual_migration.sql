-- Manual migration for hierarchical storage locations
-- Run this in Supabase SQL Editor

-- Add new columns to storage_locations table
ALTER TABLE storage_locations
ADD COLUMN IF NOT EXISTS parent_id UUID REFERENCES storage_locations(id) ON DELETE CASCADE,
...
CHECK (type IN ('Pantry', 'Freezer', 'Refrigerator', 'DeepFreezer', 'Shelf', 'Section', 'Drawer', 'Compartment'));
...
CREATE INDEX IF NOT EXISTS idx_storage_locations_household_level ON storage_locations(household_id, level);
...
-- Create function to get full location path
