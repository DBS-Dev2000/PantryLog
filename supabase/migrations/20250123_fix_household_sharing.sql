-- Fix household data sharing and add recipe privacy settings
...
-- Products can be shared across households (global products) or household-specific
DO $$
BEGIN
...
CREATE POLICY "Everyone can view products"
    ...
    USING (true); -- Products are global
...
-- 12. Create helper function to get user's households
...
-- 13. Create helper function to check household membership
...
BEGIN
    RETURN EXISTS (
        ...
        AND household_members.household_id = check_household_id
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DO $$
BEGIN
    RAISE LOG 'Household sharing migration completed successfully';
END $$;