...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'meal_history' AND policyname = 'meal_history_policy') THEN
    CREATE POLICY meal_history_policy ON meal_history
      FOR ALL USING (household_id = auth.uid());
  END IF;

  -- member_dietary_restrictions policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'member_dietary_restrictions' AND policyname = 'member_dietary_restrictions_policy') THEN
    CREATE POLICY member_dietary_restrictions_policy ON member_dietary_restrictions
      FOR ALL USING (household_id = auth.uid());
  END IF;

  -- food_preferences policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'food_preferences' AND policyname = 'food_preferences_policy') THEN
    CREATE POLICY food_preferences_policy ON food_preferences
      FOR ALL USING (household_id = auth.uid());
  END IF;

  -- household_schedules policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'household_schedules' AND policyname = 'household_schedules_policy') THEN
    CREATE POLICY household_schedules_policy ON household_schedules
      FOR ALL USING (household_id = auth.uid());
  END IF;

  -- household_meal_preferences policies
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'household_meal_preferences' AND policyname = 'household_meal_preferences_policy') THEN
